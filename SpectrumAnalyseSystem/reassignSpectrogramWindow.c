...
static float g_reassignSpectrogramStartMs = -1.0f;
static unsigned char g_reassignSpectrogramStartFlag = 0;
...
void updateReassignSpectrogramWindow(GtkWidget* widget, cairo_t* cr, XSPECTRO* spectro, SMousePos* mousePos) {
    ...
    drawReassignSpectrogram(widget, cr, spectro, mousePos);
    reassignSpectrogramDrawGrid(widget, cr, spectro, mousePos);
    reassignSpectrogramDrawMouseInformation(widget, cr, spectro, mousePos);
}
...
void drawReassignSpectrogram(GtkWidget* widget, cairo_t* cr, XSPECTRO* spectro, SMousePos* mousePos) {
    ...
    float _ms, _ms0;
    _ms0 = 0.0f;
    if (g_reassignSpectrogramStartMs < 0.0f) {
        ...
        g_reassignSpectrogramStartMs = _ms0;
    }
    else {

        if (_saveTime < g_reassignSpectrogramStartMs || _saveTime >= g_reassignSpectrogramStartMs + _specms) {
            ...
            _ms0 = g_reassignSpectrogramStartMs;
        }
    }
    ...
    g_reassignSpectrogramStartFlag = 0;
    if (fabs(_ms0) < 1E-5) {
        ...
        g_reassignSpectrogramStartFlag = 2;
    }
    else {

        _width = _wndWidth - _x0 * 2;
        g_reassignSpectrogramStartFlag = 1;
    }
    ...
    for (_i = 0; _i < _height; _i++) {
        ...
        for (_j = 0; _j < _width; _j++) {
            ...
            if (_validX && _validY) {
                ...
                char _gv = spectro->reallgvs[_pos];
...
// Draw the grid for reassign spectrogram window
void reassignSpectrogramDrawGrid(GtkWidget* widget, cairo_t* cr, XSPECTRO* spectro, SMousePos* mousePos) {
    ...
    if (g_reassignSpectrogramStartFlag > 0)
        _startLen = 0.0f;
...
#ifdef SAS_VS2022
        sprintf_s(_chDatas, 255, "%d", (int)(g_reassignSpectrogramStartMs + (float)_i * 100.0f + 0.5f));
#else
    ...
    if (g_reassignSpectrogramStartMs + _specms > _totms) {

        _x = (float)_x0 + _startLen + (float)(_width - _startLen) * (_totms - g_reassignSpectrogramStartMs) / _specms;
    ...
    if (g_reassignSpectrogramStartFlag == 0)
        _x += 10.0f;
    _x += _width * (_saveTime - g_reassignSpectrogramStartMs) / _specms;
